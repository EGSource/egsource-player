 ...
 *       {"name":"CLICK","src":"https://.../click.mp3"}
 *     ]'>
 *   </eg-stem-player>
 *
...
(function () {
  ...
  // ---- iOS speaker-unlock workaround ------------------------------------
  // iOS Safari has a documented quirk where the Web Audio API can silently
  // fail to route to the device's built-in speakers (while working fine
  // over Bluetooth/wired headphones) unless a native <audio> element is
  // also played during the same user-gesture. This tiny silent WAV primes
  // that route on the first Play tap.
  ...
  function unlockIOSAudio(audioCtx) {
    ...
    // Also nudge the AudioContext itself with a near-silent buffer burst.
    try {
...
`;
  ...
  function parseNoteName(input) {
    ...
    const isFlat = /^[A-G]b$/.test(s);
    let lookup = s;
    if (FLAT_ALIASES[lookup]) lookup = FLAT_ALIASES[lookup];
    const idx = SHARP_NAMES.indexOf(lookup);
    if (idx === -1) return null;
    return { index: idx, useFlats: isFlat };
  }

  function noteName(idx, useFlats) {
  ...
  class EGStemPlayer extends HTMLElement {
    ...
    _ensureCtx() {
      if (!this._ctx) {
        // Tell Safari this page's audio is real "playback" content (music),
        // not transient/ambient sound effects. By default Safari treats
        // Web Audio API output as "ambient", which is why it can fight the
        // speaker route and cut out shortly after starting — this is the
        // proper fix, not a workaround. Supported on modern Safari; safely
        // ignored elsewhere.
        ...
        // iOS can suspend the audio session mid-playback for reasons
        // outside our control (hardware volume button presses, brief
        // interruptions, etc.) without the page ever calling pause().
        // Auto-resume if that happens while we still think we're playing,
        // so it self-heals instead of silently going dead.
        this._ctx.addEventListener('statechange', () => {
          if (this._isPlaying && this._ctx.state === 'suspended') {
            this._ctx.resume().catch(() => {});
          }
        });

        // Backup watchdog: some iOS versions don't reliably fire
        // 'statechange' for every interruption, so also poll periodically.
        setInterval(() => {
          if (this._isPlaying && this._ctx.state === 'suspended') {
            this._ctx.resume().catch(() => {});
          }
    ...
    async loadTracks(trackDefs) {
      ...
      const loads = trackDefs.map(async (def, i) => {
        ...
        // Tracks named "CLICK" (click/metronome/cue tracks) are exempt from
        // transpose — they should always play at their original pitch
        // regardless of the song's transpose setting.
        const noTranspose = /click/i.test(def.name);
        this._tracks[i].noTranspose = noTranspose;
        pitch.setPitchSemitones(noTranspose ? 0 : this._semitones);
    ...
    setSongKey(keyName) {
      const parsed = parseNoteName(keyName);
      this._songKeyIndex = parsed ? parsed.index : null;
      this._songKeyUseFlats = parsed ? parsed.useFlats : false;
      this._els.transposeLabel.textContent = this._formatSemitones(this._semitones);
    }

    setTransposeSemitones(n) {
      this._semitones = n;
      this._tracks.forEach(t => t.pitch && !t.noTranspose && t.pitch.setPitchSemitones(n));
      this._els.transposeLabel.textContent = this._formatSemitones(n);
    }

    _formatSemitones(n) {
      if (this._songKeyIndex !== null) {
        const useFlats = this._songKeyUseFlats;
        const from = noteName(this._songKeyIndex, useFlats);
        if (n === 0) return `Original Key (${from})`;
        const to = noteName(this._songKeyIndex + n, useFlats);
        const sign = n > 0 ? '+' : '';
    ...
    play() {
      ...
      const start = () => {
        ...
        // If a loop section is active, pressing Play always (re)starts
        // from the beginning of that section, even if the person clicked
        // around elsewhere while paused. Turning the loop toggle off
        // restores normal behavior: resume from wherever it was left.
        if (this._loop && this._loopRegion) {
          this._pausedAt = this._loopRegion.start;
        }
      ...
      if (ctx.state === 'suspended') {
        // iOS Safari can leave the audio context suspended even after a
        // user gesture; ctx.currentTime does not advance while suspended,
        // which is why playback can look "active" in the UI but stay
        // silent with a frozen timestamp. Wait for resume() to actually
    ...
    // Like seek(), but always starts playback from the new position —
    ...
    // Defines a section (in seconds) to repeat — drag on the timeline to
    // set this interactively, or call directly for a programmatic loop.
    // Automatically enables looping.
    setLoopRegion(startSec, endSec) {
      if (!this._duration) return;
      const start = Math.max(0, Math.min(startSec, endSec));
      const end = Math.min(this._duration, Math.max(startSec, endSec));
      if (end - start < 0.05) { this.clearLoopRegion(); return; }
      this._loopRegion = { start, end };
      this._loop = true;
      this._els.loopBtn.classList.add('active');
      this._els.loopClearBtn.style.display = 'flex';
      this._previewLoopRegion(start / this._duration, end / this._duration);

      // Start playing the new section immediately — no separate click
      // needed after dragging to hear it.
      if (this._isPlaying) {
        this._tracks.forEach(t => t.pitch && t.pitch.disconnectOut());
        this._isPlaying = false;
      }
      this._pausedAt = start;
      this._updateTimeDisplay(start);
      this._drawAllWaveforms();
      this.play();
    }

    // Removes the practice loop section, reverting to whole-song playback
    // (still looped if the loop toggle remains on).
    clearLoopRegion() {
      this._loopRegion = null;
      this._els.loopClearBtn.style.display = 'none';
      this._els.loopRegionEl.style.display = 'none';
      this._tracks.forEach(t => {
        if (t.els && t.els.loopRegionEl) t.els.loopRegionEl.style.display = 'none';
      });
    }
    ...
    _runProgressLoop() {
      const step = () => {
        ...
        if (this._loop && this._loopRegion && pos >= this._loopRegion.end) {
          this.seek(this._loopRegion.start);
          return;
        }
        if (this._loop && !this._loopRegion && pos >= this._duration) {
          this.seek(0);
          return;
        }
        if (!this._loop && pos >= this._duration) {
          this.pause();
          this._pausedAt = 0;
          this._updateTimeDisplay(0);
          return;
    ...
    // Returns 0-1: how "tall" a track's waveform should render, based on
    // its volume slider and whether it's currently audible (mute/solo).
    // Fully muted (or soloed-out) collapses to a flat line; volume scales
    // the rest linearly, so the waveform visually shrinks as you pull a
    // fader down.
    _trackAmplitudeScale(i) {
      const t = this._tracks[i];
      if (!t) return 1;
      const anySolo = this._tracks.some(tt => tt.solo);
      const audible = anySolo ? t.solo : !t.mute;
      if (!audible) return 0;
      return t.pitch ? t.pitch.volume : 1;
    }
    ...
    _render() {
      ...
      style.textContent = `
        @import url('https://fonts.googleapis.com/css2?family=Manrope:wght@400;600;700&family=JetBrains+Mono:wght@400;500&display=swap');
        :host { display:block; font-family: 'Manrope', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; }
        ...
        .song-title { font-size:15px; font-weight:700; margin-right:8px; white-space:nowrap; }
        .time { font-family: 'JetBrains Mono', monospace; font-size:13px; color:${COLORS.subtext}; min-width:36px; text-align:center; }
        ...
        .loop-region { position:absolute; top:-7px; height:17px; background:rgba(34,255,222,0.22); border-left:2px solid ${COLORS.accent}; border-right:2px solid ${COLORS.accent}; border-radius:2px; display:none; pointer-events:none; }
        .loop-clear-btn { background:none; border:none; color:${COLORS.subtext}; cursor:pointer; font-size:16px; line-height:1; padding:2px 4px; display:none; }
        .loop-clear-btn:hover { color:${COLORS.text}; }
        .transpose { display:flex; align-items:center; gap:8px; padding-left:8px; border-left:1px solid #3a3a3a; }
        .transpose-label { font-family: 'JetBrains Mono', monospace; font-size:12px; color:${COLORS.subtext}; min-width:110px; text-align:center; }
        ...
        .track-name { color:${COLORS.text}; font-size:13px; font-weight:700; width:110px; flex-shrink:0; letter-spacing:0.3px; }
        .no-transpose-tag { display:block; font-family:'Manrope',sans-serif; font-size:9px; font-weight:400; color:${COLORS.subtext}; letter-spacing:0.2px; margin-top:2px; }
        .waveform-wrap { flex:1; height:44px; position:relative; }
        .waveform-wrap:hover { outline: 1px solid #3a3a3a; }
        .waveform-loop-region { position:absolute; top:0; height:100%; background:rgba(34,255,222,0.18); border-left:2px solid ${COLORS.accent}; border-right:2px solid ${COLORS.accent}; display:none; pointer-events:none; }
      ...
      wrap.innerHTML = `
        <div class="transport">
          ...
          <button class="loop-clear-btn" data-role="loopClear" title="Clear loop section">\u2715</button>
          ...
          <div class="scrubber" data-role="scrubber">
            <div class="scrubber-track">
              ...
              <div class="loop-region" data-role="loopRegion"></div>
        ...
        <div class="credit">EGSource PracticeMix</div>
      `;
      ...
      this._els = {
        ...
        loopClearBtn: wrap.querySelector('[data-role="loopClear"]'),
        loopRegionEl: wrap.querySelector('[data-role="loopRegion"]'),
      ...
      this._attachScrubDrag(this._els.scrubber);
      this._els.loopClearBtn.addEventListener('click', () => {
        this.clearLoopRegion();
      });
    }

    // Shared drag-to-select behavior for the main scrubber AND every
    // waveform canvas: a plain click seeks/plays from that point (same as
    // before); an actual drag defines a practice loop section. All 6
    // surfaces (scrubber + 5 waveforms) share one timeline, so a drag on
    // any of them updates the same loop region and stays visually synced
    // across all of them.
    _attachScrubDrag(el) {
      let drag = null;
      el.addEventListener('pointerdown', (e) => {
        ...
        const startFrac = Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width));
        drag = { startFrac, curFrac: startFrac, moved: false };
      ...
      el.addEventListener('pointermove', (e) => {
        ...
        if (Math.abs(curFrac - drag.startFrac) > 0.005) {
          drag.moved = true;
          drag.curFrac = curFrac;
          const a = Math.min(drag.startFrac, curFrac);
          const b = Math.max(drag.startFrac, curFrac);
          this._previewLoopRegion(a, b);
        }
      });
      el.addEventListener('pointerup', (e) => {
        if (!drag) return;
        if (drag.moved) {
          const a = Math.min(drag.startFrac, drag.curFrac);
          const b = Math.max(drag.startFrac, drag.curFrac);
          this.setLoopRegion(a * this._duration, b * this._duration);
        } else {
          this.scrubTo(drag.startFrac * this._duration);
        }
        drag = null;
      });
    }

    // Shows an in-progress loop-region preview (while dragging) across the
    // main scrubber and every waveform simultaneously, before the drag is
    // finalized into an actual loop via setLoopRegion.
    _previewLoopRegion(startFrac, endFrac) {
      const left = (startFrac * 100) + '%';
      const width = ((endFrac - startFrac) * 100) + '%';
      this._els.loopRegionEl.style.display = 'block';
      this._els.loopRegionEl.style.left = left;
      this._els.loopRegionEl.style.width = width;
      this._tracks.forEach(t => {
        if (!t.els || !t.els.loopRegionEl) return;
        t.els.loopRegionEl.style.display = 'block';
        t.els.loopRegionEl.style.left = left;
        t.els.loopRegionEl.style.width = width;
      });
    }
      ...
      names.forEach((name, i) => {
        ...
        row.innerHTML = `
          ...
          <div class="waveform-wrap">
            ...
            <div class="waveform-loop-region" data-role="waveformLoopRegion"></div>
          </div>
        `;
        ...
        const loopRegionEl = row.querySelector('[data-role="waveformLoopRegion"]');
        canvas.style.cursor = 'pointer';
        this._attachScrubDrag(canvas);

        this._tracks[i].els = { row, soloBtn, muteBtn, volSlider, canvas, loopRegionEl };
    ...
    _paintWaveform(i, playedFrac) {
      ...
      const ampScale = this._trackAmplitudeScale(i);
      for (let x = 0; x < peaks.length; x++) {
        const amp = ampScale <= 0.01 ? 1 : Math.max(2, peaks[x] * (h - 4) * ampScale);
